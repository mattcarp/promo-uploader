...
export const inputElement = document.getElementById("file-select");

export class DragAndDrop {
  htmlElements = new elements.HtmlElements();
  helper = new Helper();
  ...
  onClickRegister() {
    dropZone.addEventListener("click", () => {
      if (dropZone.className !== "disabled") {
        inputElement.click();
        this.htmlElements.cleanThumbnails();
      }
    });
  }
    ...
    dropZone.addEventListener("change", (e) => {
      ...
      if (inputElement.files.length && dropZone.className !== "disabled") {
        for (let i = 0; i < dropZoneFile.files.length; i++) {
          this.updateThumbnail(dropZone, inputElement.files[i]);
          store.addFileToFileList({
            row: "new",
            fileInfo: dropZoneFile.files[i],
            uploaded: false,
          });
          store.addToTotalSize(dropZoneFile.files[i].size);
        }

        elements.btnUpload.removeAttribute("disabled");
        this.htmlElements.btnActivate();
        this.htmlElements.cleanProgressBar();
        this.htmlElements.updateFileList();
        elements.progressTotal.innerHTML = "Ready for upload";
      }
    });
  }
    ...
    dropZone.addEventListener("drop", (e) => {
      ...
      if (e.dataTransfer.files.length && dropZone.className !== "disabled") {
        ...
        for (let i = 0; i < e.dataTransfer.files.length; i++) {
          this.updateThumbnail(dropZone, e.dataTransfer.files[i]);
           store.addFileToFileList({
             row: "new",
             fileInfo: e.dataTransfer.files[i],
             uploaded: false,
           });
           store.addToTotalSize(e.dataTransfer.files[i].size);
        }

        this.htmlElements.btnUpload.removeAttribute("disabled");
        this.htmlElements.btnActivate();
        this.htmlElements.cleanProgressBar();
        this.htmlElements.updateFileList();
        this.htmlElements.progressTotal.innerHTML = "Ready for upload";
  ...
  updateThumbnail(dropZoneElement, file) {
    ...
    thumbnailElement.dataset.label = this.helper.smartTrim(file.name, 14);
